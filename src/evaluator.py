...
def format_metric_html(
    ...
    return f"""
    <div style="margin: 10px 0; padding: 18px; background-color: #e8f4f8; border-radius: 12px; border-left: 7px solid {color};">
        <div style="font-size: 15px; color: #444; margin-bottom: 7px;">{label}</div>
...
def run_retrieval_evaluation(progress=gr.Progress()):
    ...
    <div style="padding: 0;">
        ...
        <div style="margin-top: 20px; padding: 12px; background-color: #fff3cd; border-radius: 8px; text-align: center; border: 2px solid #ffc107;">
            <span style="font-size: 15px; color: #856404; font-weight: bold;">✓ Evaluation Complete: {count} tests</span>
        </div>
    </div>
    """
...
def main():
    ...
    theme = gr.themes.Monochrome(font=["Inter", "system-ui", "sans-serif"])

    with gr.Blocks(title="RAG Evaluation Dashboard", theme=theme) as app:
        gr.Markdown("# 📊 RAG Evaluation Dashboard")
        gr.Markdown("Evaluate retrieval and answer quality for the EP1001 documentation RAG system")

        # RETRIEVAL SECTION
        gr.Markdown("## 🔍 Retrieval Evaluation")

        retrieval_button = gr.Button("Run Evaluation", variant="primary", size="sm")

        with gr.Row():
            with gr.Column(scale=1):
                retrieval_metrics = gr.HTML(
                    "<div style='padding: 20px; text-align: center; color: #6c757d;'>Click 'Run Evaluation' to start</div>"
                )
        ...
        answer_button = gr.Button("Run Evaluation", variant="primary", size="xl")

        with gr.Row():
            with gr.Column(scale=1):
                answer_metrics = gr.HTML(
                    "<div style='padding: 20px; text-align: center; color: #495057;'>Click 'Run Evaluation' to start</div>"
                )
